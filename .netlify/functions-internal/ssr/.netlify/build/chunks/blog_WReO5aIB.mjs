/* empty css                         */
import { c as createComponent, r as renderTemplate, m as maybeRenderHead, d as renderComponent, a as addAttribute } from './astro/server_6rCQxYeE.mjs';
import 'kleur/colors';
import 'html-escaper';
...
}, "D:/webs/Panambi_Landing/src/components/Icons/ArticleIcon.astro", void 0);
...
We use an agile approach to test assumptions and connect with the
    ...
    ({
      ...
      author,
      authorImage
    }) => renderTemplate`<article class="p-6 bg-white rounded-3xl border border-gray-200 shadow-card"> <div class="flex justify-between items-center mb-5 text-gray-500"> <span class="bg-lime text-black text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded"> ${type == "Article" ? renderTemplate`${renderComponent($$result3, "ArticleIcon", $$ArticleIcon, {})}` : renderTemplate`${renderComponent($$result3, "VideoIcon", $$VideoIcon, {})}`} ${type} </span> <span class="text-sm">${publish_date}</span> </div> <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900"> ${title} </h2> <p class="mb-5 font-light text-gray-500" style="white-space: pre-line"> ${brief} </p> <div class="flex justify-between items-center"> <div class="flex items-center space-x-4"> <img class="w-7 h-7 rounded-full"${addAttribute(authorImage, "src")}${addAttribute(author + " cover", "alt")}> <span class="font-medium">${author}</span> </div> <a${addAttribute(`/blogs/${id}`, "href")} class="inline-flex items-center font-medium text-black hover:text-lime">
Read more
...
}, "D:/webs/Panambi_Landing/src/pages/blog.astro", void 0);

const $$file = "D:/webs/Panambi_Landing/src/pages/blog.astro";
const $$url = "/blog";
